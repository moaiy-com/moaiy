# Engineering Documentation Index

This directory contains the core engineering and release documentation for Moaiy.

Language policy:
- Engineering docs: English
- User-facing docs: English + Chinese where required
- Chinese README: `README_CN.md`

## Core Architecture & Product Engineering

### [Project Info](./project-info.md)
- Current release status and repository references
- Localization strategy and supported UI languages

### [Technical Architecture](./technical-architecture.md)
- Layered system design
- Core services and data models
- Security, performance, and testing strategy

### [Design System](./design-system.md)
- Color, typography, spacing, component conventions
- UI behavior and accessibility guidance

### [Extreme Usability Design](./extreme-usability-design.md)
- Product UX direction for low-friction encryption workflows
- Interaction principles and measurable usability targets

### [Xcode Integration Guide](./xcode-integration-guide.md)
- Project integration notes
- Build and configuration guidance

### [Pro Open Core Contracts v1](./pro-open-core-contracts-v1.md)
- Public/private repository boundaries for Pro
- Stable contracts, semantic versioning, and entitlement integration

## Release Operations

### [Release Workflow Guide](./release-workflow-skill.md)
- Tracked release process specification
- Workflow split, publish gate, and fallback policy

### [Notification Permission Regression Script (v0.6.0)](./v0.6.0-notification-permission-regression-script.md)
- Manual runbook for reminder permission regression validation

## Compliance & Security Engineering

### [App Store Compliance Plan](./app-store-compliance.md)
- Export compliance posture
- Sandbox strategy
- Submission and review checklist

### [Sandbox Testing Plan](./sandbox-testing-plan.md)
- App Sandbox validation matrix
- Entitlement profiles and test cases

### [Technical Validation Status](./technical-validation-status.md)
- Current validation progress and evidence
- Open risks and next actions

### [v0.5.0 Security Audit](./v0.5.0-security-audit.md)
- Lightweight release audit findings and mitigations

## Bundled GPG Track

### [Bundled GPG Development Plan](./bundled-gpg-development-plan.md)
### [Bundled GPG Quick Start](./bundled-gpg-quick-start.md)
### [Bundled GPG Summary](./bundled-gpg-summary.md)
### [Bundled GPG Completion Report](./bundled-gpg-completion-report.md)

## App Store Subdocuments

### [Export Compliance Mapping](./app-store/export-compliance.md)
### [Privacy Manifest Mapping](./app-store/privacy-manifest-mapping.md)

## Historical Release Documentation (v0.5.0)

### [Minimal Release Checklist](./v0.5.0-minimal-release-checklist.md)
### [Core Flow Validation](./v0.5.0-core-flow-validation.md)
### [Release Blockers](./v0.5.0-release-blockers.md)
### [D4 Regression Matrix](./v0.5.0-d4-regression-matrix.md)
### [D4 Manual Regression Script](./v0.5.0-d4-manual-regression-script.md)
### [External Validation Pack](./v0.5.0-external-validation-pack.md)
### [v0.5.0 Release Notes](./v0.5.0-release-notes.md)

## Root-Level References

- [Project README](../README.md)
- [Contribution Guide](../CONTRIBUTING.md)
- [Security Policy](../SECURITY.md)
- [Changelog](../CHANGELOG.md)
- [Latest Release (v0.6.5)](https://github.com/moaiy-com/moaiy/releases/tag/v0.6.5)
- [Disclaimer](../DISCLAIMER.md)

---

Last updated: 2026-04-18
